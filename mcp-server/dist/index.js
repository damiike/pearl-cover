        ...
        execute: async (params) => {
            try {
                ...
                if (types.includes('notes'))
                    filtered.notes = results.notes.slice(0, params.limit || 10);
                if (types.includes('claims'))
                    filtered.claims = results.claims.slice(0, params.limit || 10);
                if (types.includes('aged_care_expenses'))
                    filtered.agedCareExpenses = results.agedCareExpenses.slice(0, params.limit || 10);
                if (types.includes('workcover_expenses'))
                    filtered.workcoverExpenses = results.workcoverExpenses.slice(0, params.limit || 10);
                if (types.includes('payments'))
                    filtered.payments = results.payments.slice(0, params.limit || 10);
                if (types.includes('attachments'))
                    filtered.attachments = results.attachments.slice(0, params.limit || 10);
                // Count total results
